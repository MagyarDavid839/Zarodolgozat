...
if(!empty($_POST["login"]) && !empty($_POST["email"]) && !empty($_POST["password"])) {	
	...
	if($customer->login()) {
		...
		$loginMessage = 'Nem megfelelő adatok!';
	}
} else {
	$loginMessage = 'Tölts ki mindent!';
}
endif;
    ...
    <div class="col-lg-4 col-md-5 col-sm-10 col-xs-12">
        <div class="card card-info rounded-0">
            <div class="card-header">
                <div class="card-title h4 mb-0 fw-bold text-center">Bejelentkezés</div>
            </div>
                ...
                <form id="loginform" class="form-horizontal" role="form" method="POST" action="">
                    ...
                    <div class="text-center">
                        <div class="col-sm-12 controls">
                            <input type="submit" name="login" value="Belépés" class="btn btn-primary rounded-0">
                            <a href="./register.php" class="btn btn-light bg-gradient border rounded-0">Regisztráció</a>
                        </div>
                    </div>
                </form>
            </div>
